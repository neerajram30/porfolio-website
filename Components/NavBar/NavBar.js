...
export default function NavBar() {
  ...
  return (
    <div>
      <nav className="bg-[#fafafc] shadow-[#eeeeee3d] shadow-nav h-14 md:fixed fixed w-full z-10">
        {open && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed flex flex-col items-center justify-center z-20 w-screen h-screen inset-0 bg-gray-200 dark:bg-gray-800"
          >
            <motion.button
        ...
        <div className="md:flex space-x-3 justify-between">
          <div className="md:flex text-white dark:text-white pt-5 pl-2 md:ml-12">
            {links.map((link) => (
              <Link href={'#'+ link.id} passHref className='text-black ml-5 font-extrabold md:block hidden'>
